...
1. Simply run `make` to build the application. After building you will need to 
give the binary the `CAP_IPC_LOCK` capability, required for it to lock memory 
pages for real-time safety.

    ```bash
    make
    sudo setcap cap_ipc_lock=+ep gpio-toggle
    ```
